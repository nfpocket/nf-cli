  ...
  const crudTypesResponse = await inquirer.prompt([
    {
      ...
      choices: [
        ...
        {
          name: `${modelName}/[id]/index.delete.ts  | Delete a record`,
          value: "delete",
          checked: true,
        },
        {
          name: `${modelName}/[id]/index.get.ts     | Get a record`,
          value: "getById",
          checked: true,
        },
        {
          name: `${modelName}/[id]/index.put.ts     | Update a record`,
          value: "put",
          checked: true,
        },
      ],
...
const crudTypeToFileName = (crudType) => {
  switch (crudType) {
    ...
    case "delete":
      return "[id]/index.delete.ts";
    case "getById":
      return "[id]/index.get.ts";
    case "put":
      return "[id]/index.put.ts";
  }
};
...
const generateCrud = async (crudPath, modelName, crudTypes) => {
  ...
  for (const crudType of crudTypes) {
    ...
    try {
      ...
      if (!fs.existsSync(path.join(process.cwd(), fullPathDir))) {
        fs.mkdirSync(path.join(process.cwd(), fullPathDir), { recursive: true });
      }
